...
<?php
        $emid=$_REQUEST['emid'];
        $q="SELECT * FROM chat WHERE sender='$emid' AND receiver='admin@gmail.com' OR receiver='$emid' AND sender='admin@gmail.com' ORDER BY `chatid`";
        $s= mysqli_query($con, $q);
        while ($r= mysqli_fetch_array($s))
        {
if($r[1]=="admin@gmail.com"){    
echo"
...
if (isset($_POST['submit'])){
  ...
  $sql="INSERT INTO `chat` (`sender`,`receiver`,`message`,`date`) VALUES('admin@gmail.com','$emid','$msg',(select sysdate()))";
   echo $sql;
   if (mysqli_query($con,$sql)){
    echo "<script>alert('Messaged Successfully')</script>";
    echo "<script>location.href='querydetail.php?emid=$emid'</script>";
